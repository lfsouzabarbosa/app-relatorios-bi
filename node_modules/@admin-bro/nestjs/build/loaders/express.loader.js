...
let ExpressLoader = class ExpressLoader extends abstract_loader_1.AbstractLoader {
    register(admin, httpAdapter, options) {
        ...
        load_package_util_1.loadPackage('express', '@admin-bro/nestjs');
        const adminBroExpressjs = load_package_util_1.loadPackage('@admin-bro/express', '@admin-bro/nestjs', () => require('@admin-bro/express'));
        load_package_util_1.loadPackage('express-formidable', '@admin-bro/nestjs');
        let router;
        if ('auth' in options) {
            load_package_util_1.loadPackage('express-session', '@admin-bro/nestjs');
            router = adminBroExpressjs.buildAuthenticatedRouter(admin, options.auth, undefined, options.sessionOptions, options.formidableOptions);
        }
        else {
